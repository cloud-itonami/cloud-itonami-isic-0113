(ns vegops.operation
  ...
  supervised actor run. The advisor (VegOpsAdvisor) is sealed into a
  single node (:advise); its proposal is ALWAYS routed through the Field
  ...
  One run = one vegetable/root-crop-growing coordination operation
  (intake -> advise -> govern -> decide -> commit | hold | approval). No
  unbounded inner loop -- each operation is auditable and checkpointed. A
  field's operating history is advanced by MANY operations
  (log-field-record / schedule-field-operation / flag-crop-health-concern
  ...
  is clean -- see `vegops.governor/always-escalate-ops`.

  NOTE: langgraph-clj StateGraph integration is deferred (mirrors
  `cerealops.operation`, cloud-itonami-isic-0111). This stub version
  ...
  (:require [vegops.advisor :as advisor]
